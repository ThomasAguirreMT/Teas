const links = {
  Servicios: ["Fibra Hogar", "Fibra Empresarial", "IP Fija", "Soporte Técnico"],
  Empresa:   ["Nosotros", "Cobertura", "Blog", "Trabaja con nosotros"],
  Legal:     ["Términos de servicio", "Política de privacidad", "PQR", "Tarifas"],
};
    ...
    <footer style={{ background:"#16202b" }}>
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-10">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center"
                ...
                <div className="flex items-end gap-[3px] h-4">
                  {[2,3,4,5].map((h,i) => (
                    <div key={i} className="w-[3px] rounded-sm bg-white" style={{ height:`${h*3}px` }} />
                  ))}
                </div>
              </div>
            ...
            <div className="flex gap-3">
              {["f","in","tw","yt"].map(s => (
                <div key={s} className="w-8 h-8 rounded-full flex items-center justify-center text-xs cursor-pointer"
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8"
          ...
          <p className="text-xs" style={{ color:"rgba(255,255,255,0.3)" }}>© 2025 TEAS S.A.S. Todos los derechos reservados.</p>
          <p className="text-xs" style={{ color:"rgba(255,255,255,0.3)" }}>NIT: 900.123.456-7 · Vigilado por MinTIC</p>
        </div>
      </div>
    </footer>
  );
