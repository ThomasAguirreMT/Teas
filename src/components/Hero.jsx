...
export default function HeroCarousel() {
  ...
  const [speed, setSpeed]     = useState(slides[0].speed);
  ...
  useEffect(() => {
    setSpeed(0);
    const t = setTimeout(() => setSpeed(slides[current].speed), 50);
  ...
  const tk = isGamer
    ...
    : {
        sectionBg:    "linear-gradient(160deg, #0f1923 0%, #162030 50%, #111c2a 100%)",
        ...
        cardValColor: "#f1f5f9",
        cardLblColor: "#64748b",
        dialTrack:    "rgba(255,255,255,0.08)",
        dialStop1:    "#00ae9d",
        dialStop2:    "#2d4258",
        dialNum:      "#f1f5f9",
        dialUnit:     "#00ae9d",
  ...
  return (
    ...
    >
      ...
      {isGamer ? (
        ...
        <>
          <div className="absolute pointer-events-none rounded-full"
            style={{ width: 500, height: 500, background: "rgba(0,174,157,0.07)", filter: "blur(100px)", top: -120, right: -100 }} />
          <div className="absolute pointer-events-none rounded-full"
            style={{ width: 400, height: 400, background: "rgba(45,66,88,0.15)", filter: "blur(80px)", bottom: -80, left: -60 }} />
        </>
      )}
      ...
      <div className="w-full max-w-6xl mx-auto px-6 md:px-16 pt-24 pb-12 relative">

        {/* ── Layout: apilado en mobile, grid en desktop ── */}
        <div className="flex flex-col md:grid md:grid-cols-2 md:gap-16 md:items-center">
          ...
          <div key={`text-${current}`} className="flex flex-col justify-center">
            ...
            {/* Badge */}
            <div
              className="inline-flex items-center gap-2 px-4 py-1.5 mb-6 border self-start"
              style={{
                background:    tk.badgeBg,
                borderColor:   tk.badgeBorder,
                color:         tk.badgeColor,
                borderRadius:  isGamer ? "4px" : "9999px",
                letterSpacing: "0.1em",
                textTransform: "uppercase",
                fontSize:      "0.7rem",
                fontWeight:    800,
                boxShadow:     isGamer ? "0 0 12px rgba(168,85,247,0.3)" : "none",
              }}
            >
              <span className="w-1.5 h-1.5 rounded-full animate-pulse"
                style={{ background: tk.pulseColor }} />
              {slide.badge}
            </div>
            ...
            <h1
              ...
              style={{
                ...
                color:         tk.titleColor,
                letterSpacing: isGamer ? "-0.03em" : "-0.01em",
                textTransform: isGamer ? "uppercase" : undefined,
                textShadow:    isGamer ? "0 0 40px rgba(168,85,247,0.4)" : "none",
              }}
            >
              {slide.title}
            </h1>
            ...
            <p className="leading-relaxed mb-8 max-w-md"
              style={{ color: tk.descColor, fontSize: "clamp(0.9rem, 2vw, 1.05rem)" }}>
              {slide.desc}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-6 border-t"
              ...
              {slide.checks.map(b => (
                <div key={b} className="flex items-center gap-2 text-xs font-medium"
                  style={{ color: tk.descColor }}>
                  <span style={{ color: tk.checkColor }}>{isGamer ? "■" : "✓"}</span> {b}
                </div>
              ))}
            </div>
          </div>
          ...
          {isGamer ? (
            <div
              key={`gamer-${current}`}
              className="flex flex-col gap-3 mt-10 md:mt-0"
            >
            ...
            >
              {/* Velocímetro — más pequeño en mobile */}
              <div className="relative w-44 h-44 md:w-60 md:h-60">
                <svg viewBox="0 0 200 200" className="w-full h-full"
                  style={{ transform: "rotate(-135deg)" }}>
                  <defs>
                    <linearGradient id="sg" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%"   stopColor={tk.dialStop1} />
                      <stop offset="100%" stopColor={tk.dialStop2} />
                    </linearGradient>
                  </defs>
                  <circle cx="100" cy="100" r="80" fill="none"
                    stroke={tk.dialTrack} strokeWidth="12" />
                  <circle cx="100" cy="100" r="80" fill="none"
                    stroke="url(#sg)" strokeWidth="12" strokeLinecap="round"
                    strokeDasharray={`${(speed / maxSpd) * 376} 502`}
                    style={{ transition: "stroke-dasharray 0.4s ease" }}
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="font-display font-extrabold leading-none"
                    style={{ fontSize: "clamp(2rem,5vw,3rem)", color: tk.dialNum }}>
                    {speed}
                  </span>
                  <span className="text-sm font-semibold mt-1" style={{ color: tk.dialUnit }}>
                    Mbps
                  </span>
                  <span className="text-xs mt-2" style={{ color: tk.dialLabel }}>
                    {slide.speedLabel}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-6 w-full max-w-xs">
                {slide.metrics.map(m => (
                  <div key={m.label} className="shimmer rounded-2xl p-4 border"
                    style={{
                      background:  tk.cardBg,
                      borderColor: tk.cardBorder,
                      boxShadow:   "0 2px 8px rgba(0,0,0,0.2)",
                    }}>
                    <div className="text-lg mb-1" style={{ color: tk.checkColor }}>{m.icon}</div>
                    <div className="font-semibold text-sm" style={{ color: tk.cardValColor }}>{m.val}</div>
                    <div className="text-xs mt-0.5" style={{ color: tk.cardLblColor }}>{m.label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center justify-center gap-4 mt-10">
          ...
          {/* Flechas visibles solo en mobile */}
          <button
            onClick={() => goTo(current - 1)}
